...
def main():
    ...
    driver.get(base_url)
    login(driver)
    original_window_handle = navigate(driver)
    browse_pages(driver, original_window_handle)
    driver.quit()
...
def browse_pages(driver, original_window_handle):
    while True:
        ...
        next_disabled = driver.find_element(by=By.XPATH, value="/html/body/main/div[2]/div/div/div/div[2]/div/div/div/div/div[2]/div[4]/div/ul/li[5]")
        if(next_disabled):
            break
        else:
...
def write_job_info_to_csv(job_info, csv_file):
